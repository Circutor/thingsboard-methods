...
func TestFailBodyEncodeSaveEntityAttributesV1(t *testing.T) {
	...
	// Send Telemetry mock
	entityType := "USER"
	entityID := "adb386d0-a37f-11eb-a40e-6b088d7482e1"
	scope := "SERVER_SCOPE"
...
func TestFailRequestSaveEntityAttributesV1(t *testing.T) {
	...
	request.On("CreateNewRequest", "POST",
		"/api/plugins/telemetry/"+entityType+"/"+entityID+"/"+scope, "Bearer token_value", respBody, query).
		Return(nil, 500, errors.NewErrFound("error in create request"))

	controller := telemetry.NewControllerTelemetryMock("/", "", "", data, request)

	status, _, _ := controller.SaveEntityAttributesV1(entityType, entityID, scope, "Bearer token_value", attrBody)
	assert.Equal(t, http.StatusInternalServerError, status)
}
