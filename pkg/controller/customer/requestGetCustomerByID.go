...
func (c *ControllerCustomer) GetCustomerByID(id, token string) (int, map[string]interface{}, error) {
	url := c.TB.URLTBServer + customer + "/" + id
	...
	if message, ok := responseBody["message"]; ok {
		...
		return status, dataError, errors.NewErrFound(fmt.Sprint(thingsBoard), fmt.Sprint("GetCustomerByID ->", message))
	}
